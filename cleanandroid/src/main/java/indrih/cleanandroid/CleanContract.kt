...
interface CleanContract {
    ...
    interface Presenter<Event : AbstractEvent> {
         ...
         * Является suspend функцией, которая запускается в основном потоке, т.к.
         * не должна нарушать этапы жизненного цикла в случае необходимости запустить
         * продолжительные инициализирующие действия при первом добавлении.
         */
        suspend fun onFirstAttached()
