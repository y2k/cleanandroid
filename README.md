...
## Предисловие
Данная библиотека является аналогом Moxy, призванная избавиться от излишней кодогенерации и дать автоматическое 
управление жизненным циклом корутин. Моё более подробное видение архитектуры можно найти в 
...
fun notifyMain(event: Event, mainEvent: MainEvent) {
    when (mainEvent) {
        ...
        else -> Unit // добавлен как индикатор того, для всех ли событий была прописана реализация
...
3) Для перехода на нужный экран в Presenter-е вызываем:
   ...
   // с параметрами
   router.navigateTo(SomeScreen(device))
   ```
